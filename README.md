...
- 🔐 配置存独立文件，代码不含任何真实凭据
- 🚫 **智能跳过**：已自动删帖/封禁的命中不推送（帖子已被规则处理，无需人工关注）；仅 `manual_confirm`（需人工确认）或未自动处理的命中才推送
...
**访问方式**：在浏览器打开 `http://<你的WTM地址>:36799/push-notify`（WTM 服务端口，默认 36799）。`<你的WTM地址>` 取决于你的部署环境：

- 本机访问：`http://127.0.0.1:36799/push-notify`
- 局域网/组网内访问：用你的内网 IP（如蒲公英组网虚拟 IP）
- ⚠️ **注意**：内网地址属于你的私有网络信息，**不要写进公开文档、仓库或截图**；对外发布/分享时一律用 `<wtm-host>` 占位符
...
**验证边界**：本插件在**中小流量贴吧**实际运行验证（作者自己的贴吧流量较小）。**大流量贴吧场景未充分测试**，可能存在未发现的问题（高并发命中、推送风暴、配额耗尽等）。如果你要在大吧使用，请先在低流量环境充分测试。

原因：
- **推送量仍可能大**：大吧帖子命中频繁。虽然已自动删帖的命中会被跳过，但命中率高时（尤其 `manual_confirm` 或未自动处理的规则多），推送量依然可观
- **配额压力**：Server酱免费版每天仅 5 条，大吧命中量远超配额，会触发限流或静默失败
- **适用场景**：中小型贴吧、低命中率规则、需要人工关注的违规内容监控

如果确实需要在大吧使用，建议：
- 规则设置更精准（高价值关键词，而非宽泛词）
- 使用邮件通道（无限条数）而非 Server酱
- 配合 WTM 的 `record_all_context` 关闭等设置，降低推送频率
- 让规则直接 `delete`（自动删帖）——这样命中后插件会自动跳过推送，减少打扰

## 工作原理

插件在加载时运行时包装（monkey-patch）`Processer.process` 方法：WTM 爬虫处理帖子时，命中规则的那一瞬间触发推送——**不轮询数据库，零延迟**。

命中后推送前，插件会检查该规则是否**自动执行了删帖/封禁**（`delete` / `block` / `delete_and_block` 且非 `manual_confirm`）：
- 已自动处理 → **跳过推送**（帖子已被删，无需人工关注）
- 需人工确认（`manual_confirm: true`）或未自动处理 → **正常推送**（提醒人工处理）
